...
export const Appointment = ({
  ...
  if (appointmentObject.employeeAppointments.length > 0) {
    const appointmentEmployeeRelationship =
      appointmentObject.employeeAppointments[0];
    assignedEmployee = employees.find(
      (employee) => employee.id === appointmentEmployeeRelationship.employeeId
    );
  }
  ...
  const canDelete = () => {
    return (
      <Button
        ...
        className="appointmentDelete__button"
      >
        Delete
      </Button>
    );
  ...
  const canClaim = () => {
    if (nailedItUserObject.staff) {
      return appointmentObject.employeeAppointments.length ? (
        ...
        <Button
          variant="dark"
          className="appointmentClaim__button"
          onClick={() => {
  ...
  const canEdit = () => {
    if (appointmentObject.dateCompleted === dateCompleted()) {
      ...
      return (
        <Button
          variant="dark"
          className="appointmentEdit__button"
          onClick={() => navigate(`/appointments/${appointmentObject.id}/edit`)}
        >
          Edit
        </Button>
      );
  ...
  return (
    <>
      <section
        ...
        key={`appointment--${appointmentObject.id}`}
      >
        ...
        <div className="appointment__details">
          <p className="appointment__text">
            Color: {appointmentObject.nailColor.color}
          </p>
          <p className="appointment__text">
            Shape: {appointmentObject.nailShape.shape}
          </p>
          <p className="appointment__text">
            Effect: {appointmentObject.nailEffect.effect}
          </p>
          <p className="appointment__text">
            Directions: {appointmentObject.directions}
          </p>
          <p className="appointment__text">
            Date Booked: {appointmentObject.dateBooked}
          </p>
